...
- media provider 选择、替换与元数据更新
- 文章操作审计日志记录
- 文章归档/恢复生命周期与批量治理
- 后续可继续承接更细粒度的恢复策略、审计扩展和发布流
...
- 管理端运营计数优先通过 repository 聚合快照返回，再由 query 层补充少量业务语义组合，避免页面层反复拼接多次 `count` 查询
- 管理端运营计数应区分“基础快照”和“派生运营语义”：例如 `ready` 这类来源于 `review + 可发布草稿` 的值，不应伪装成数据库基础快照本身
- 后台首页总览卡片与治理债务卡片，应优先使用两套投影结果，而不是让页面直接消费一个持续膨胀的全量 counts 对象
